...
bool ObjectFinder::find_upright_coke_can(float surface_height, geometry_msgs::PoseStamped &object_pose) {
    ...
    double block_height = 0.15; //this height is specific to the TOY_BLOCK model
    //if insufficient points in plane, find_plane_fit returns "false"
    //should do more sanity testing on found_object status
    //hard-coded search bounds based on a block of width 0.15
    found_object = pclUtils_.find_plane_fit(0.84, 0.86, 0.39, 0.41, surface_height + 0.14, surface_height + 0.16, 0.001,
